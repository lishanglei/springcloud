 ...
 */
@Slf4j
@ControllerAdvice(basePackages = "com.mingbyte.baoneng")
public class GlobalResponseHandler implements ResponseBodyAdvice<Object> {
    ...
    public boolean supports(MethodParameter methodParameter, Class<? extends HttpMessageConverter<?>> aClass) {
        ...
        若必须返回单个字符串，需要使用 GlobalResponseWrapper.success("result")
         */
    ...
    public Object beforeBodyWrite(Object o, MethodParameter methodParameter, MediaType mediaType, Class<? extends HttpMessageConverter<?>> aClass, ServerHttpRequest serverHttpRequest, ServerHttpResponse serverHttpResponse) {
        ...
        if ("void".equals(returnTypeName)) {
            return GlobalResponseWrapper.success();
        }

        /*
        若返回值不是 json 类型则直接返回
        否则会出现类型转换异常
        接口不能直接返回字符串类型，否则默认的转换器无法进行解析并报错，若必须返回单个字符串，需要使用 GlobalResponseWrapper.success("result") , 在前面的判断会直接将包装器返回
         */
        ...
        if (GlobalExceptionCode.class.getName().equals(returnTypeName)) {
            return GlobalResponseWrapper.ok((GlobalExceptionCode) o);
        }

        return GlobalResponseWrapper.success(o);
    }
}
